  ...
  const addFiles = (files) => {
    setFormState({
      ...
      documents: formState.documents
        ? formState.documents.concat(files)
        : files,
    });
  };
    ...
    <Card className="mb-5">
      ...
      <Card.Body>
        <div className="mb-4">
          ...
          grades for the modules you have declared. Tutors who have provided us
          with supporting documents will earn a verified certificate that will
          be displayed on their profile.
        </div>
        Note: Limit of 5 files (image or PDF files, each with ~7MB file size
        limit)
        <DropzoneArea
          showPreviews={formState.documents && formState.documents.length !== 0}
